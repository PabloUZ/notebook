...
While a DTO validates and shapes data coming **into** a controller, a DAO shapes and filters the data going **out** — the response actually sent to the client. It uses `class-transformer`, the same package that powers a DTO's `@Type()` decorators.

## 1. Dependencies

`class-transformer` is already installed if you followed the DTO setup — no extra package is required for DAOs.

## 2. Create DAO Files

### 2.1. File Naming Convention

Create DAO files with the extension `.dao.ts`, inside a `dao/` folder:
