...
Add a plain object of constants inside `src/config`, next to the rest of the app-wide configuration:
...
Every microservice and the API Gateway are **independent NestJS projects**, so there's no shared package to import `patterns.ts` from. Keep a copy of the file in every project that needs it, and update every copy whenever a pattern changes — exactly the same rule already used for the gRPC `.proto` contract.
