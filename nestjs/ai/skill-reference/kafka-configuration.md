...
> **Note:** `KAFKA_BROKERS` can hold a comma-separated list (e.g. `kafka1:9092,kafka2:9092`). In production, these are injected automatically by Docker Compose's `env_file`.
...
Then add a `kafka` block to `envs.type.ts`, splitting the brokers list:
