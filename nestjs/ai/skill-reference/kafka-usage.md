...
How to expose topic handlers in a Kafka microservice and how to consume them from an API Gateway.
...
Why `subscribeToResponseOf()` is required before `connect()`: without it, the reply topic listener isn't set up before requests are sent, so `send()` calls that expect a response would never resolve.
