...
Learn how to expose queue handlers in your RabbitMQ microservice, how to manually acknowledge messages, and how to consume them from an API Gateway.
...
- How to expose request-response and event handlers over a RabbitMQ queue
- How to manually acknowledge or requeue messages with `RmqContext`
- How to register a `ClientProxy` in the API Gateway and call the microservice

---

## Next Steps

Once your gateway and microservice can talk to each other, orchestrate them (and the RabbitMQ broker) with Docker Compose.
