...
> **Note:** In production, these are injected automatically by Docker Compose's `env_file` — see [Docker Setup for Microservices](../docker-setup.md).
...
Then add a `rabbitmq` block to `envs.type.ts` (see [Environment Variables](../../setup/environment-variables.md)):
...
Now that your microservice can start and connect to RabbitMQ, expose its handlers.

**Continue with:** [Using RabbitMQ Microservices](./usage.md)
