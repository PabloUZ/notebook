...
> [!IMPORTANT]
> Never place `docker-compose.yml` inside a service's folder. It must sit at the root, above every project, so it can reference all of them by relative path. Each service keeps its **own** `.env.dev`/`.env.prod`/`.env.example` inside its own folder — there's no shared `.env` at the root. See [Dockerization](../setup/dockerization.md#4-configure-environment-files) for the `.gitignore` pattern.
...
For development, mirror it in `docker-compose.dev.yml` with each service's `Dockerfile.dev` and its own source volume, following the same pattern as [Dockerization](../setup/dockerization.md):
...
- Point each service's transport `host`/`url` (see [TCP](./tcp/configuration.md), [gRPC](./grpc/configuration.md)) to the **container name** of the target service, not `localhost`:
...
If any of your services use [Kafka](./kafka/configuration.md), add the broker to the Compose file (KRaft mode, no separate Zookeeper needed):
...
If any of your services use [RabbitMQ](./rabbitmq/configuration.md), add the broker to the Compose file:
...
- Why Compose files must sit one level above every service's folder
- How to build each service from its own subfolder with `build.context`
- How services reach each other by container name over the internal Docker network
- How to add and wait on the Kafka and RabbitMQ brokers
