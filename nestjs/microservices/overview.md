...
Learn how to split a NestJS application into independent microservices that communicate with each other, instead of a single app talking to a database.

---

## 1. Project Structure

Every microservice (including the API Gateway) is its **own independent NestJS project**, created with `nest new`. This guide does **not** use Nest's monorepo mode (`nest generate app`) — each service has its own `package.json`, its own dependencies, its own Dockerfile, and can be deployed and scaled independently.
...
Each service, on its own, still follows the rest of this guide: [Setup](../setup/create-project.md), [Database](../database/docker-setup.md), [Validation](../validation/dto.md), etc. This section only covers what's specific to inter-service communication and orchestration.
...
> **Tip:** Define pattern strings like `'sum'` or `'user_created'` as constants instead of repeating the literal across every microservice and the gateway — see [Microservice String Constants](../architecture/microservices-constants.md).
...
Pick the transport you want to use for your microservice:

- **[TCP](./tcp/configuration.md)** — the simplest, no external broker
- **[gRPC](./grpc/configuration.md)** — strongly-typed contracts over HTTP/2
- **[Kafka](./kafka/configuration.md)** — event streaming
- **[RabbitMQ](./rabbitmq/configuration.md)** — message queues
